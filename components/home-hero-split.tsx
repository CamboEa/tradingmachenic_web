  ...
  return (
    <div className="flex items-center justify-center gap-1.5" aria-hidden>
      {Array.from({ length: 5 }).map((_, i) => (
        <span
          key={i}
          className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-[color-mix(in_oklab,var(--color-teal)_35%,var(--color-bridge))] bg-[color-mix(in_oklab,var(--color-teal)_12%,white)] shadow-sm"
        >
        ...
        <div className="relative flex min-h-[min(92vh,44rem)] flex-col lg:min-h-[min(88vh,40rem)] lg:flex-row">
          ...
          <div className="relative z-20 flex flex-1 flex-col justify-center px-5 py-14 sm:px-8 lg:w-[min(46%,32rem)] lg:flex-none lg:shrink-0 lg:py-20 xl:pl-12">
            <div
              ...
              aria-hidden
            />
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_20%_30%,color-mix(in_oklab,var(--color-teal)_22%,transparent),transparent_55%)] opacity-90" aria-hidden />
            <div className="relative">
              <p className="text-xs font-bold uppercase tracking-[0.28em] text-[var(--color-gold)]">
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <Link
                  ...
                  className={[
                    "inline-flex w-fit items-center justify-center rounded-lg px-8 py-3.5 text-sm font-bold shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl",
                    titleCaps ? "uppercase tracking-[0.12em]" : "",
                    "bg-[var(--color-teal)] text-white shadow-[color-mix(in_oklab,var(--color-teal)_35%,transparent)] hover:brightness-105",
                  ].join(" ")}
                >
                  {h.heroSplitCta}
                </Link>
