...
export function isNavActive(href: string, pathname: string) {
  ...
  if (href === "/admin/mentors") {
    return (
      pathname === "/admin/mentors" ||
      (pathname.startsWith("/admin/mentors/") && !pathname.startsWith("/admin/mentor-accounts"))
    );
  }
...
export function buildAdminNav(access: { role: "admin" | "mentor"; mentorSlug?: string } | null): NavSection[] {
  if (!access || access.role === "admin") return NAV;

  const mentorHref = `/admin/mentors/edit/${access.mentorSlug}`;
  return [
    {
      heading: "My content",
      items: [
        { href: mentorHref, label: "Profile & lessons", icon: mentorIcon },
...
export function adminPanelLabel(access: { role: "admin" | "mentor" } | null) {
  return access?.role === "mentor" ? "Mentor Panel" : "Admin Panel";
}
