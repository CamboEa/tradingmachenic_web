...
export function AdminSidebar() {
  ...
  return (
    <aside className="hidden h-screen w-64 shrink-0 flex-col border-r border-white/10 bg-[#1e293b] text-slate-300 lg:flex">
      {/* Logo */}
                ...
                return (
                  <li key={item.href}>
                    ...
                    >
                      <span
                        className={[
                          "absolute left-0 top-1/2 h-5 w-0.5 -translate-y-1/2 rounded-full bg-[#2563EB] transition-opacity",
                          active ? "opacity-100" : "opacity-0",
                        ].join(" ")}
                        aria-hidden
                      />
                      <span className={active ? "text-[#2563EB]" : "text-slate-500 group-hover:text-slate-300"}>
                        {item.icon}
                      </span>
                      {item.label}
                    </Link>
      ...
      <div className="border-t border-white/10 px-3 py-3">
        <Link
          href="/"
          className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold text-slate-400 transition-colors hover:bg-white/5 hover:text-white"
        >
