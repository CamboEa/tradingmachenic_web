...
export function HomeHeroSplit({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  return (
    ...
    >
      ...
      {/* Ambient glow */}
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_70%,color-mix(in_oklab,#0EA5E9_14%,transparent),transparent)]"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_50%_40%_at_50%_50%,color-mix(in_oklab,#D4AF37_8%,transparent),transparent)]"
        aria-hidden
      />
      ...
      <div className="relative flex h-full flex-col items-center justify-center px-4 text-center sm:px-6 lg:px-8">
        ...
        >
          {(() => {
            ...
            return (
              <>
                {rest}{" "}
                <span className="bg-linear-to-r from-[#D4AF37] via-[#F0C855] to-[#D4AF37] bg-clip-text text-transparent">
                  {last}
                </span>
              </>
            );
        ...
        <div className="mt-7 flex items-center gap-2" aria-hidden>
          ...
          <span className="h-1 w-1 rounded-full bg-[#0EA5E9]/50" />
        ...
        >
          <Link
            ...
            className="group relative inline-flex items-center gap-2 overflow-hidden rounded-xl bg-[#0EA5E9] px-8 py-3.5 text-sm font-bold uppercase tracking-[0.12em] text-white shadow-[0_8px_32px_rgba(14,165,233,0.35)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_40px_rgba(14,165,233,0.50)]"
          >
      ...
      <div className="absolute bottom-8 left-1/2 flex -translate-x-1/2 flex-col items-center gap-2">
        ...
        <div className="relative h-8 w-px overflow-hidden bg-white/10">
          <span
            className="absolute inset-x-0 top-0 h-full bg-gradient-to-b from-transparent via-white/60 to-transparent"
