// SuperAggressiveEA v1.0 (MQL5 / MT5) — high-risk preset: InitialLot 1.0, side TP $50, daily profit stop $1000
#property copyright "SuperAggressiveEA"
#property description "SuperAggressiveEA v1.0 — high risk: L1 1.0 lot, $50/side TP, $1000/day equity profit stop"
...
input int    GridSmoothMaxDeltaPips = 10; // Adaptive grid: max pip change toward target per tick (0 = snap instantly)
...
input bool   UseL1MomentumConfirm   = true; // L1 only: last closed bar on L1MomentumTF vs EMA(Trend period)
input ENUM_TIMEFRAMES L1MomentumTF  = PERIOD_M1;
...
input double MaxSpreadPipsForEntry = 0.3; // Block NEW entries when (ask-bid)/pip exceeds this; 0 = off (matches Strategy Tester / live spread)

//=== HEDGE OVERRIDE =================================================
input int    HedgeOverrideLevels = 3;  // Ignore trend filter when opposing grid >= N levels
input double HedgeOverrideMinAtrMult = 1.0; // When override active + filter blocks: require adverse move vs avg >= this * ATR (GridVolatilityTF ATR); 0 = off

//=== PROFIT INPUTS ==================================================
input double BuyTakeProfitUSD  = 50.0;
input double SellTakeProfitUSD = 50.0;
...
input ulong  MagicNumber   = 20260520;
...
//=== MONDAY ENTRY DELAY (SERVER TIME) ===============================
input bool UseMondayEntryDelay     = true;  // Block NEW entries from Mon 00:00 server for N minutes
input int  MondayEntryDelayMinutes = 150;   // Default 2h30; e.g. 120=2h, 180=3h

//=== PROFIT STOP (CLOSE + PAUSE) ====================================
input bool   UseProfitStop      = true;    // Stop trading once daily equity gain >= ProfitStopUSD
input double ProfitStopUSD      = 1000.0;  // Daily equity gain vs day open; pause when reached

//=== DAILY PROFIT DISPLAY ===========================================
input double DailyProfitTargetUSD = 1000.0;  // Dashboard; default matches ProfitStopUSD
...
int      g_l1MomEmaHandle     = INVALID_HANDLE;
...
void RefreshDayStartEquity()
{
   ...
   if(g_dayKey == 0 || g_dayKey != key)
   {
      ...
      // New day: reset profit-stop baseline + latch (daily profit stop mode)
      g_startEquity = eq;
...
void RefreshGridVolatilityCache()
{
   ...
   int target = (int)MathRound((atr[0] * GridATR_Multiplier) / g_pip);
   if(target < GridStepMinPips) target = GridStepMinPips;
   if(target > GridStepMaxPips) target = GridStepMaxPips;

   int diff = target - g_effGridPips;
   if(diff == 0)
      return;

   if(GridSmoothMaxDeltaPips <= 0)
   {
      g_effGridPips = target;
      return;
   }

   int mag = (int)MathMin(MathAbs(diff), GridSmoothMaxDeltaPips);
   if(mag < 1)
      mag = 1;
   g_effGridPips += (diff > 0 ? mag : -mag);
}
...
bool CanEnterNewTrade(string &why)
{
   ...
   string mon = "";
   if(IsMondayEntryDelayActive(mon)) { why = mon; return false; }

   string nd = "";
   if(IsNewsBlackout(nd)) { why = "news"; return false; }

   if(MaxSpreadPipsForEntry > 0.0)
   {
      double spr = SpreadPipsNow();
      if(spr > MaxSpreadPipsForEntry)
      {
         why = "spread";
         return false;
      }
...
bool L1MomentumConfirmBuy()
{
   if(!UseL1MomentumConfirm)
      return true;
   if(g_l1MomEmaHandle == INVALID_HANDLE)
      return true;

   double ema[1];
   if(CopyBuffer(g_l1MomEmaHandle, 0, 1, 1, ema) != 1)
      return false;

   double c = iClose(_Symbol, L1MomentumTF, 1);
   if(c == 0.0)
      return false;
...
bool HedgeOverrideExtraAllowsSell(double buyAvg, double bid)
{
   if(HedgeOverrideMinAtrMult <= 0.0)
      return true;
   if(!g_gridATROK || buyAvg <= 0.0 || g_gridATRValue <= 0.0)
      return true;

   double need = HedgeOverrideMinAtrMult * g_gridATRValue;
   return ((bid - buyAvg) >= need);
}

bool HedgeOverrideExtraAllowsBuy(double sellAvg, double ask)
{
   if(HedgeOverrideMinAtrMult <= 0.0)
      return true;
   if(!g_gridATROK || sellAvg <= 0.0 || g_gridATRValue <= 0.0)
      return true;

   double need = HedgeOverrideMinAtrMult * g_gridATRValue;
   return ((sellAvg - ask) >= need);
}

void ImmediateRefreshSideTP(ENUM_POSITION_TYPE side)
{
   double lots = 0.0;
   double wsum = 0.0;
   ...
   if(lots > 0.0)
      UpdateSideTPs(side, lots, wsum / lots);

   g_lastTpslTick = GetTickCount();
}
...
void CreateDashboard()
{
   ...
   ObjectSetInteger(0, n, OBJPROP_YSIZE,       40 * DASH_RH + 12);
...
void UpdateDashboard()
{
   ...
   DashLabel("hdr", lx, y, "SuperAggressiveEA v1.0   " + _Symbol, clrGold, 10);
   y += rh + 4;
   ...
   double sprNow = SpreadPipsNow();
   bool sprGateOff = (MaxSpreadPipsForEntry <= 0.0);
   bool sprOk = sprGateOff || (sprNow <= MaxSpreadPipsForEntry);
   string sprMaxStr = sprGateOff ? "off" : DoubleToString(MaxSpreadPipsForEntry, 2);
   DashLabel("lbl_sprd", lx, y, Pad("Spread", LW), cLbl);
   DashLabel("val_sprd", bx, y, DoubleToString(sprNow, 2) + " / max " + sprMaxStr, sprOk ? cGood : cWarn);
   y += rh;
   ...
   string monR = "";
   bool monBlk = IsMondayEntryDelayActive(monR);
   DashLabel("lbl_mon", lx, y, Pad("Mon entries", LW), cLbl);
   DashLabel("val_mon", bx, y, monBlk ? Trunc(monR, 36) : "OK", monBlk ? cWarn : cGood);
   y += rh;
   ...
   int effP = GetGridStepPips();
   string gapTxt = IntegerToString(effP) + " pip "
                   + (UseAdaptiveGrid ? "(ATR smooth)" : "(fixed)")
                   + "  base=" + IntegerToString(GridStep)
                   + " dmax=" + IntegerToString(GridSmoothMaxDeltaPips)
   ...
   DashLabel("val_hovs", bx,y,
             IntegerToString(HedgeOverrideLevels)+" lv | ov ATRx"+DoubleToString(HedgeOverrideMinAtrMult,1),
             cVal);
}
...
datetime GetThisMondayServerMidnight(const datetime now)
{
   MqlDateTime dt;
   TimeToStruct(now, dt);
   if(dt.day_of_week != 1)
...
bool IsMondayEntryDelayActive(string &reason)
{
   reason = "n/a";
   if(!UseMondayEntryDelay || MondayEntryDelayMinutes <= 0)
      return false;
   datetime mon0 = GetThisMondayServerMidnight(TimeCurrent());
   if(mon0 <= 0)
      return false;
   datetime until = mon0 + (datetime)MondayEntryDelayMinutes * 60;
   if(TimeCurrent() < until)
   {
      reason = "wait until " + TimeToString(until, TIME_DATE|TIME_MINUTES);
      return true;
   }
   return false;
}
...
int OnInit()
{
   ...
   g_l1MomEmaHandle = INVALID_HANDLE;
   if(UseL1MomentumConfirm)
   {
      g_l1MomEmaHandle = iMA(_Symbol, L1MomentumTF, EMA_Period, 0, MODE_EMA, PRICE_CLOSE);
      if(g_l1MomEmaHandle == INVALID_HANDLE)
         Print("WARN: L1 momentum EMA handle failed; momentum confirm skipped.");
   }
   ...
   Print("=== SuperAggressiveEA v1.0 | ",_Symbol,
   ...
   Print("=== Adaptive grid: ",UseAdaptiveGrid?"ON":"OFF",
         ...
         " | clamp ",GridStepMinPips,"-",GridStepMaxPips," | smooth d=", GridSmoothMaxDeltaPips,
   ...
   Print("=== L1 momentum: ", UseL1MomentumConfirm ? "ON" : "OFF",
         " | TF=", EnumToString(L1MomentumTF), " ===");
   Print("=== Hedge override extra: ATRmult=", DoubleToString(HedgeOverrideMinAtrMult, 2), " (0=off) ===");
   Print("=== EntryLimit: cooldown=", CooldownSeconds, "s | 1/bar=", OneEntryPerBar ? "ON" : "OFF",
         " | maxSpreadPips=", (MaxSpreadPipsForEntry > 0.0 ? DoubleToString(MaxSpreadPipsForEntry, 2) : "OFF"), " ===");
   Print("=== NewsFilter (XAUUSD/USD): ",UseNewsFilter?"ON":"OFF",
         " | +/-min=",NewsMinutesBefore,"/",NewsMinutesAfter,
         " | minImp=",IntegerToString((int)NewsMinImportance),
         " | refresh=",NewsRefreshSeconds,"s ===");
   Print("=== Monday entry delay: ", UseMondayEntryDelay ? "ON" : "OFF",
         " | after Mon 00:00 server wait ", MondayEntryDelayMinutes, " min ===");
   Print("=== RiskCaps: MaxLevelsPerSide=", MaxLevelsPerSide, " (0=unlimited) | TPSLUpdateMs=", TPSLUpdateMs,
         " | FilterLogSeconds=", FilterLogSeconds, " ===");

   double l1eff = GetGridLot(1);
   double vmin = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   double vmax = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
   double vstep = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
   Print("=== Lots: InitialLot(input)=", DoubleToString(InitialLot, 2),
         " | L1 used=", DoubleToString(l1eff, 2),
         " | symbol min/max/step=", DoubleToString(vmin, 2), "/", DoubleToString(vmax, 2), "/", DoubleToString(vstep, 2), " ===");
   if(l1eff + 1e-8 < InitialLot)
      Print("WARN: L1 lot is below InitialLot — check SYMBOL_VOLUME_MAX in tester/symbol spec, or Inputs tab (Reset if stale).");
...
void OnDeinit(const int reason)
{
   ...
   if(g_l1MomEmaHandle!=INVALID_HANDLE) IndicatorRelease(g_l1MomEmaHandle);
   if(g_gridATRHandle!=INVALID_HANDLE) IndicatorRelease(g_gridATRHandle);
   Print("=== SuperAggressiveEA v1.0 Stopped. Reason=",reason," ===");
}
...
void OnTick()
{
   ...
   if(g_emaCacheOk)
   {
      ...
      {
         if(buyBlocked)
            Print("FILTER: Buy blocked  [", bA ? "dist" : "slope", "] ask=", DoubleToString(ask,_Digits),
                  " ema=", DoubleToString(ema,_Digits), " sp=", DoubleToString(sp,1), "p");
         if(sellBlocked)
            Print("FILTER: Sell blocked [", sA ? "dist" : "slope", "] bid=", DoubleToString(bid,_Digits),
                  " ema=", DoubleToString(ema,_Digits), " sp=", DoubleToString(sp,1), "p");
   ...
   if(buyCount==0 && sellCount==0)
   {
      ...
      if(wantBuy && wantSell)
      {
         ...
         {
            double sp = g_emaSlopePips;
            double tie = (double)EMA_MinSlopePips * 0.5;
            if(sp > tie)
               wantSell = false;
            else if(sp < -tie)
               wantBuy = false;
      ...
      if(wantBuy && !L1MomentumConfirmBuy())
         wantBuy = false;
      if(wantSell && !L1MomentumConfirmSell())
         wantSell = false;

      if(wantBuy)
      {
         double lot=GetGridLot(1);
         Print("SuperAggressiveEA v1.0 | BUY L1 | lot=",lot," | sell arms@",DoubleToString(ask+GetGridGapPrice(),_Digits));
         if(trade.Buy(lot,_Symbol,0,0,0,"GRID BUY L1"))
         {
            MarkEntry();
            ImmediateRefreshSideTP(POSITION_TYPE_BUY);
         }
      }
      else if(wantSell)
      {
         double lot=GetGridLot(1);
         Print("SuperAggressiveEA v1.0 | SELL L1 | lot=",lot," | buy arms@",DoubleToString(bid-GetGridGapPrice(),_Digits));
         if(trade.Sell(lot,_Symbol,0,0,0,"GRID SELL L1"))
         {
            MarkEntry();
            ImmediateRefreshSideTP(POSITION_TYPE_SELL);
         }
      }
      else
      {
         datetime now=TimeCurrent();
         if(now-g_lastClosePrint>=60){Print("FILTER: L1 blocked / chop — waiting.");g_lastClosePrint=now;}
      }
      return;
   }
   ...
   if(sellCount==0 && buyCount>0)
   {
      ...
      if(bid>=trigger)
      {
         bool sellAllowed = !sellBlocked || (hov && HedgeOverrideExtraAllowsSell(buyAvg, bid));
         if(sellAllowed && !LevelAlreadyOpen(POSITION_TYPE_SELL,trigger))
         {
            if(!allowEntry) return;
   ...
   if(buyCount==0 && sellCount>0)
   {
      ...
      if(ask<=trigger)
      {
         bool buyAllowed = !buyBlocked || (hov && HedgeOverrideExtraAllowsBuy(sellAvg, ask));
         if(buyAllowed && !LevelAlreadyOpen(POSITION_TYPE_BUY,trigger))
         {
            if(!allowEntry) return;
            if(MaxLevelsPerSide > 0 && 1 > MaxLevelsPerSide)
            {
