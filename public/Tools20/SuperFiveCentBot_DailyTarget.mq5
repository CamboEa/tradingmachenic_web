...
void ResetSideStats(SideStats &s)
{
   ...
   s.hi    = 0.0;
   s.avg   = 0.0;
   ArrayResize(s.tickets, 0);
}

void PushTicket(SideStats &s, ulong tk)
{
...
//+------------------------------------------------------------------+
//  OnTick helpers — split out for clarity and easier tuning
//+------------------------------------------------------------------+
bool TryFridayShutdown()
{
   string friReason = "";
   if(!IsFridayShutdownActive(friReason)) return false;
   CloseAllPositions();
   datetime now = TimeCurrent();
...
void MaybeUpdateBasketTPs()
{
   uint nowMs = GetTickCount();
   bool tpslDue = (TPSLUpdateMs <= 0) || (nowMs - g_lastTpslTick) >= (uint)TPSLUpdateMs;
   bool stateChanged =
      (MathAbs(g_snap.buy.lots  - g_lastBuyLots ) > 0.0000001) ||
      (MathAbs(g_snap.sell.lots - g_lastSellLots) > 0.0000001) ||
      (MathAbs(g_snap.buy.avg   - g_lastBuyAvg  ) > (2.0 * _Point)) ||
      (MathAbs(g_snap.sell.avg  - g_lastSellAvg ) > (2.0 * _Point)) ||
      (g_effGridPips != g_lastEffGridPips);

   if(!(tpslDue || stateChanged)) return;

   if(g_snap.buy.count  > 0) UpdateSideTPs(POSITION_TYPE_BUY,  g_snap.buy.lots,  g_snap.buy.avg);
   if(g_snap.sell.count > 0) UpdateSideTPs(POSITION_TYPE_SELL, g_snap.sell.lots, g_snap.sell.avg);
   g_lastTpslTick    = nowMs;
   g_lastBuyLots     = g_snap.buy.lots;
   g_lastSellLots    = g_snap.sell.lots;
   g_lastBuyAvg      = g_snap.buy.avg;
   g_lastSellAvg     = g_snap.sell.avg;
   g_lastEffGridPips = g_effGridPips;
}

bool TryCloseBaskets()
{
   bool didClose = false;
   if(g_snap.buy.count > 0 && g_snap.buy.pnl >= BuyTakeProfitUSD)
   {
      Print("BUY TP hit — closing buys.");
      CloseSide(POSITION_TYPE_BUY);
      didClose = true;
   }
   if(g_snap.sell.count > 0 && g_snap.sell.pnl >= SellTakeProfitUSD)
   {
...
void ComputeTrendFilter(double ask, double bid, bool &buyBlocked, bool &sellBlocked)
{
   buyBlocked  = false;
   sellBlocked = false;
   if(!g_emaCacheOk) return;
...
bool TryOpenInitialL1(double ask, double bid, bool buyBlocked, bool sellBlocked, bool allowEntry, const string entryWhy)
{
   if(!allowEntry)
   {
      datetime now = TimeCurrent();
...
bool TryAddBuyLevel(double ask, bool allowEntry)
{
   if(g_snap.buy.count <= 0) return false;
   double trigger = g_snap.buy.lo - g_gridGapPrice;
   if(ask > trigger) return false;
   if(LevelAlreadyOpen(POSITION_TYPE_BUY, trigger)) return false;
   if(!allowEntry) return true;

   if(MaxLevelsPerSide > 0 && g_snap.buy.count >= MaxLevelsPerSide)
   {
      datetime now = TimeCurrent();
      if(now - g_lastEntryBlockPrint >= 60)
      {
         Print("RISK CAP: MaxLevelsPerSide reached for BUY (", g_snap.buy.count, "/", MaxLevelsPerSide, ") — not adding.");
         g_lastEntryBlockPrint = now;
      }
      return true;
   }
   int lvl = g_snap.buy.count + 1;
   double lot = GetGridLot(lvl);
   Print("Grid BUY L", lvl, " | lot=", lot, " | trigger=", DoubleToString(trigger,_Digits));
   if(trade.Buy(lot,_Symbol,0,0,0,"GRID BUY L"+IntegerToString(lvl))) MarkEntry();
   return true;
}

bool TryHedgeSellL1(double bid, bool sellBlocked, bool allowEntry)
{
   if(g_snap.sell.count != 0 || g_snap.buy.count <= 0) return false;
   bool hov = (g_snap.buy.count >= HedgeOverrideLevels);
   double trigger = g_snap.buy.hi + g_gridGapPrice;
   if(bid < trigger) return false;
   if(!((!sellBlocked || hov) && !LevelAlreadyOpen(POSITION_TYPE_SELL, trigger))) return false;
   if(!allowEntry) return true;
   ...
   Print("Grid SELL L1", (hov && sellBlocked ? " [hedge override]" : ""),
         " | lot=", lot, " | trigger=", DoubleToString(trigger,_Digits), " | buyCount=", g_snap.buy.count);
   if(trade.Sell(lot,_Symbol,0,0,0,"GRID SELL L1")) MarkEntry();
   return true;
}

bool TryHedgeBuyL1(double ask, bool buyBlocked, bool allowEntry)
{
   if(g_snap.buy.count != 0 || g_snap.sell.count <= 0) return false;
   bool hov = (g_snap.sell.count >= HedgeOverrideLevels);
   double trigger = g_snap.sell.lo - g_gridGapPrice;
   if(ask > trigger) return false;
   if(!((!buyBlocked || hov) && !LevelAlreadyOpen(POSITION_TYPE_BUY, trigger))) return false;
   if(!allowEntry) return true;
   ...
   Print("Grid BUY L1 (vs sell)", (hov && buyBlocked ? " [hedge override]" : ""),
         " | lot=", lot, " | trigger=", DoubleToString(trigger,_Digits), " | sellCount=", g_snap.sell.count);
   if(trade.Buy(lot,_Symbol,0,0,0,"GRID BUY L1")) MarkEntry();
   return true;
}

bool TryAddSellLevel(double bid, bool allowEntry)
{
   if(g_snap.sell.count <= 0) return false;
   double trigger = g_snap.sell.hi + g_gridGapPrice;
   if(bid < trigger) return false;
   if(LevelAlreadyOpen(POSITION_TYPE_SELL, trigger)) return false;
   if(!allowEntry) return true;

   if(MaxLevelsPerSide > 0 && g_snap.sell.count >= MaxLevelsPerSide)
   {
      datetime now = TimeCurrent();
      if(now - g_lastEntryBlockPrint >= 60)
      {
         Print("RISK CAP: MaxLevelsPerSide reached for SELL (", g_snap.sell.count, "/", MaxLevelsPerSide, ") — not adding.");
         g_lastEntryBlockPrint = now;
      }
      return true;
   }
   int lvl = g_snap.sell.count + 1;
   double lot = GetGridLot(lvl);
   Print("Grid SELL L", lvl, " | lot=", lot, " | trigger=", DoubleToString(trigger,_Digits));
   if(trade.Sell(lot,_Symbol,0,0,0,"GRID SELL L"+IntegerToString(lvl))) MarkEntry();
   return true;
}
...
void OnTick()
{
   ...
   if(TryFridayShutdown()) return;

   MaybeUpdateBasketTPs();
   if(TryCloseBaskets())        return;
   if(TryMarketCloseFilter())   return;

   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);

   bool buyBlocked, sellBlocked;
   ComputeTrendFilter(ask, bid, buyBlocked, sellBlocked);

   string entryWhy = "";
   bool allowEntry = CanEnterNewTrade(entryWhy);

   if(g_snap.buy.count == 0 && g_snap.sell.count == 0)
   {
      TryOpenInitialL1(ask, bid, buyBlocked, sellBlocked, allowEntry, entryWhy);
      return;
   }

   if(TryAddBuyLevel(ask, allowEntry))                  return;
   if(TryHedgeSellL1(bid, sellBlocked, allowEntry))     return;
   if(TryHedgeBuyL1(ask, buyBlocked, allowEntry))       return;
   if(TryAddSellLevel(bid, allowEntry))                 return;
}
...
bool LevelAlreadyOpen(ENUM_POSITION_TYPE side,double triggerPrice)
{
   ...
   {
      n=ArraySize(g_snap.buy.tickets);
      for(int i=0;i<n;i++)
      {
         ulong tk=g_snap.buy.tickets[i];
   ...
   {
      n=ArraySize(g_snap.sell.tickets);
      for(int i=0;i<n;i++)
      {
         ulong tk=g_snap.sell.tickets[i];
         if(!PositionSelectByTicket(tk)) continue;
         if(MathAbs(PositionGetDouble(POSITION_PRICE_OPEN)-triggerPrice)<hs)
            return true;
      }
...
void UpdateSideTPs(ENUM_POSITION_TYPE side, double lots, double avg)
{
   ...
   int n = (side==POSITION_TYPE_BUY) ? ArraySize(g_snap.buy.tickets) : ArraySize(g_snap.sell.tickets);
   for(int i=0;i<n;i++)
   {
      ulong tk = (side==POSITION_TYPE_BUY) ? g_snap.buy.tickets[i] : g_snap.sell.tickets[i];
      if(!PositionSelectByTicket(tk)) continue;
...
{
   int n = (side==POSITION_TYPE_BUY) ? ArraySize(g_snap.buy.tickets) : ArraySize(g_snap.sell.tickets);
   string sideTag = (side==POSITION_TYPE_BUY) ? "BUY" : "SELL";
   for(int i=0;i<n;i++)
   {
      ulong tk = (side==POSITION_TYPE_BUY) ? g_snap.buy.tickets[i] : g_snap.sell.tickets[i];
...
void CloseAllPositions()
{
   ...
   int nb = ArraySize(g_snap.buy.tickets);
   for(int i=0;i<nb;i++)
   {
      ulong tk = g_snap.buy.tickets[i];
      if(!trade.PositionClose(tk))
   ...
   int ns = ArraySize(g_snap.sell.tickets);
   for(int i=0;i<ns;i++)
   {
      ulong tk = g_snap.sell.tickets[i];
      if(!trade.PositionClose(tk))
