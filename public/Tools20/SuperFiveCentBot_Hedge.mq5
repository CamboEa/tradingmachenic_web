// SuperFiveCentBot Hedge v1.2 (MQL5 / MT5) — dual-leg grid straddle from flat
#property copyright "SuperFiveCentBot"
#property description "SuperFiveCentBot — Hedge v1.2 (dual L1 when flat; hedging account)"
...
//=== HEDGE STRADDLE (FLAT ENTRY) ====================================
// Requires MT5 account "Hedging" (netting merges opposing legs into one position).
input bool UseDualLegFlatEntry = true;  // When flat: place BUY L1 and SELL L1 in one cycle if filters allow
input bool HedgeSkipTrendOnFlat = true; // If true: skip UseTrendDirectionForL1 on flat so both legs may qualify
...
input ulong  MagicNumber   = 20240107; // distinct magic vs other SuperFiveCentBot builds
...
void UpdateDashboard()
{
   ...
   DashLabel("hdr", lx, y, "SuperFiveCentBot HEDGE v1.2   " + _Symbol, clrGold, 10);
   y += rh + 4;
...
int OnInit()
{
   ...
   Print("=== SuperFiveCentBot HEDGE v1.2 | ",_Symbol,
   ...
   Print("=== Hedge flat entry: dualLeg=",UseDualLegFlatEntry?"ON":"OFF",
         " | skipTrendOnFlat=",HedgeSkipTrendOnFlat?"ON":"OFF",
         " | need hedging-mode account ===");
   if((long)AccountInfoInteger(ACCOUNT_MARGIN_MODE) != (long)ACCOUNT_MARGIN_MODE_RETAIL_HEDGING)
      Print("WARN: Account margin mode is not retail hedging; opposing legs may net — dual grid counts can break.");
...
void OnDeinit(const int reason)
{
   ...
   Print("=== SuperFiveCentBot HEDGE v1.2 Stopped. Reason=",reason," ===");
}
...
void OnTick()
{
   ...
   if(buyCount==0 && sellCount==0)
   {
      ...
      bool trendL1 = UseTrendDirectionForL1 && g_emaCacheOk;
      if(trendL1 && !(UseDualLegFlatEntry && HedgeSkipTrendOnFlat))
      {
         double sp = g_emaSlopePips;
         double ema = g_emaNow;
      ...
      if(UseDualLegFlatEntry)
      {
         ...
         if(wantBuy)
         {
            ...
            Print("HEDGE v1.2 | BUY L1 | lot=", lot, " | sell arms@", DoubleToString(ask + gridGap, _Digits));
            okBuy = trade.Buy(lot, _Symbol, 0, 0, 0, "GRID BUY L1");
         }
         if(wantSell)
         {
            double lot = GetGridLot(1);
            Print("HEDGE v1.2 | SELL L1 | lot=", lot, " | buy arms@", DoubleToString(bid - gridGap, _Digits));
            okSell = trade.Sell(lot, _Symbol, 0, 0, 0, "GRID SELL L1");
         }
         if(okBuy || okSell)
            MarkEntry();
