// SuperScalpingEA v1.0 (MQL5 / MT5) — same logic as SuperFiveCentBot v1.1.1; default L1 lot 0.1
#property copyright "SuperScalpingEA"
#property description "SuperScalpingEA — v1.0 (SuperFiveCentBot v1.1.1 logic, InitialLot 0.1)"
...
//=== PROFIT INPUTS ==================================================
input double BuyTakeProfitUSD  = 5.0;
input double SellTakeProfitUSD = 5.0;
...
input ulong  MagicNumber   = 20260510;
...
input double ProfitStopUSD      = 500.0;   // Equity gain since EA start

//=== DAILY PROFIT DISPLAY ===========================================
input double DailyProfitTargetUSD = 500.0; // Dashboard display only
...
void CreateDashboard()
{
   ...
   ObjectSetInteger(0, n, OBJPROP_YSIZE,       38 * DASH_RH + 12);
...
void UpdateDashboard()
{
   ...
   DashLabel("hdr", lx, y, "SuperScalpingEA v1.0   " + _Symbol, clrGold, 10);
   y += rh + 4;
...
int OnInit()
{
   ...
   Print("=== SuperScalpingEA v1.0 | ",_Symbol,
...
void OnDeinit(const int reason)
{
   ...
   Print("=== SuperScalpingEA v1.0 Stopped. Reason=",reason," ===");
}
...
void OnTick()
{
   ...
   if(buyCount==0 && sellCount==0)
   {
      ...
      if(wantBuy)
      {
         ...
         Print("SuperScalpingEA v1.0 | BUY L1 | lot=",lot," | sell arms@",DoubleToString(ask+GetGridGapPrice(),_Digits));
         if(trade.Buy(lot,_Symbol,0,0,0,"GRID BUY L1")) MarkEntry();
      }
      else if(wantSell)
      {
         double lot=GetGridLot(1);
         Print("SuperScalpingEA v1.0 | SELL L1 | lot=",lot," | buy arms@",DoubleToString(bid-GetGridGapPrice(),_Digits));
